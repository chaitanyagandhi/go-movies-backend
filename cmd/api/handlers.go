...
func (app *application) registerUser(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		...
		app.errorJSON(w, errors.New("email already registered"))
		return
	}
	user.CreatedAt = time.Now()
	user.UpdatedAt = time.Now()
	uID, err := app.DB.RegisterUser(user)
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
func (app *application) moviesGraphQL(w http.ResponseWriter, r *http.Request) {
	//populate graph with movies
	movies, _ := app.DB.AllMovies()

	//get query from reques
	...
	//set the query string on variable
	g.QueryString = query
