...
func New(movies []*models.Movie) *Graph {
	...
	var fields = graphql.Fields{
		...
		"search": &graphql.Field{
			Type:        graphql.NewList(movieType),
			Description: "Seach movies by title",
			...
			Resolve: func(p graphql.ResolveParams) (interface{}, error) {
				...
				if ok {
					for _, curMovie := range movies {
						if strings.Contains(strings.ToLower(curMovie.Title), strings.ToLower(search)) {
							theList = append(theList, curMovie)
						}
					}
				}
				return theList, nil
